              ...
              for (AdaptiveContainer child in this.children) {
                ...
                if (child.constraints.withinAdaptiveConstraint(context)) {
                  ...
                  if (currentColumns < entry.columns) {
                    ...
                    int rowGutters = 0;
                    // Compute available width based on the LayoutBuilder constraints
                    // and ensure no negative widths are produced.
                    final double availableWidth =
                        (constraints.maxWidth - effectiveMargin * 2).clamp(
                          0.0,
                          double.infinity,
                        );
                    for (AdaptiveContainer rowItem in row) {
                      ...
                      double periodicWidth =
                          (availableWidth + effectiveGutter) / entry.columns;
                      ...
                      final double maxWidth = itemWidth.isFinite
                          ? itemWidth.clamp(0.0, double.infinity)
                          : 0.0;
                      ...
                      if (rowGutters < totalGutters && 1 < row.length) {
                        children.add(const SizedBox(width: 0));
                        // Add spacing as a separate widget so Wrap places it
                        children.add(
